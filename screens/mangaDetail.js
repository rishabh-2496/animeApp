...
class MangaDetail extends Component {
  ...
  pushNewScreen = () => {
    this.props.navigation.push("mangaDetail", { id: itemId });
  };

  fetchData = id => {
    const dataPromise = getMangaData(id);
    const charactersPromise = getMangaCharacters(id);
    const recommendationsPromise = getMangaRecommendations(id);
    const reviewsPromise = getMangaReviews(id);
    return Promise.all([
      dataPromise,
      charactersPromise,
